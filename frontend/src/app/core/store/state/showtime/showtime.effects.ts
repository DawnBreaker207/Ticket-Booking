...
export class ShowtimeEffects {
  ...
  loadShowtimes$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap(({movieId}) =>
        this.showtimeService.getShowtimeByMovie(movieId)
          .pipe(
            map((showtimes) => ShowtimeActions.loadShowtimesSuccess({showtimes})),
            catchError((err) => of(ShowtimeActions.loadShowtimesFailed({error: err})))
          )
      )
    )
  })
      ...
      switchMap(({id}) =>
        this.showtimeService.getShowtime(id)
          .pipe(
            ...
            catchError((err) => of(ShowtimeActions.loadShowtimeFailed({error: err})))
          )
      )
    )
  })
  createShowtime$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap(({showtime}) =>
        this.showtimeService.createShowtime(showtime)
          .pipe(
            map((showtime) => ShowtimeActions.createShowtimeSuccess({showtime})),
            catchError((err) => of(ShowtimeActions.createShowtimeFailed({error: err})))
          )
      )
    )
  })
  updateShowtime$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap(({id, showtime}) =>
        this.showtimeService.updateShowtime(id, showtime)
          .pipe(
            map((showtime) => ShowtimeActions.updateShowtimeSuccess({showtime})),
            catchError((err) => of(ShowtimeActions.updateShowtimeFailed({error: err})))
          )
      )
    )
  })
      ...
      switchMap(({id}) =>
        this.showtimeService.deleteShowtime(id)
          .pipe(
            ...
            catchError((err) => of(ShowtimeActions.deleteShowtimeFailed({error: err})))
          )
      )
    )
  })
