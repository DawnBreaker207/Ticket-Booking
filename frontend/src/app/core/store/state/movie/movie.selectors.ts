...
export const selectMovieById =
  (id: number) =>
    createSelector(selectAllMovies,
      (movies) =>
        movies.find(m => m.id === id) || null);

// Loading state selector
export const selectMovieLoading =
  createSelector(
    selectMovieState,
    (state) => state.loading);

export const selectMovieLoadingDetails =
  createSelector(
    selectMovieState,
    (state) => state.loadingDetails);

export const selectMoviesSaving =
  createSelector(
    selectMovieState,
    (state) => state.saving);
...
export const selectMoviesCount =
  createSelector(
    selectAllMovies,
    (movies) => movies.length);

export const selectHasMovies =
  createSelector(
    selectAllMovies,
