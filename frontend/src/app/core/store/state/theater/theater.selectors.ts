...
import {theaterFeatureKey, TheaterState} from '@/app/core/store/state/theater/theater.reducers';

export const selectTheaterState = createFeatureSelector<TheaterState>(theaterFeatureKey);
...
export const selectTheaterById =
  (id: number) =>
    createSelector(selectAllTheaters,
      (theaters) =>
        theaters.find(m => m.id === id) || null);
