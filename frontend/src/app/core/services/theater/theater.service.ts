...
import {Showtime, ShowtimeRequest, Theater, TheaterRequest} from '@/app/core/models/theater.model';
...
export class TheaterService {
  ...
  getTheaterByLocation(location: string) {
    ...
    return this.http.get<ApiRes<Theater>>(`${this.URL_THEATER}/search`, {params}).pipe(
      map((res) => res.data),
      catchError(this.handleError<Theater>('theater'))
    )
  }
  ...
  updateTheater(id: number, schedule: TheaterRequest) {
    return this.http.put<ApiRes<Theater>>(`${this.URL_THEATER}/${id}`, schedule).pipe(
      map((res) => res.data),
      catchError(this.handleError<Theater>('theater'))
    )
  }
  ...
  deleteTheater(id: number) {
    return this.http.delete<void>(`${this.URL_THEATER}/${id}`).pipe(
      catchError(this.handleError<void>('theater'))
    )
  }
  ...
  createShowtime(showtime: ShowtimeRequest) {
    return this.http.post<ApiRes<Showtime>>(`${this.URL_SHOWTIME}`, showtime).pipe(
      map((res) => res.data),
      catchError(this.handleError<Showtime>('showtime'))
    )
  }
  ...
  updateShowtime(id: number, showtime: ShowtimeRequest) {
    return this.http.put<ApiRes<Showtime>>(`${this.URL_SHOWTIME}/${id}`, showtime).pipe(
      map((res) => res.data),
      catchError(this.handleError<Showtime>('showtime'))
    )
  }
  ...
  deleteShowtime(id: number) {
    return this.http.delete<void>(`${this.URL_SHOWTIME}/${id}`).pipe(
      catchError(this.handleError<void>('showtime'))
    )
  }

  // Seat
  getSeatByShowtime(showtimeId: number) {
    return this.http.get<ApiRes<Showtime>>(`${this.URL_SHOWTIME}/showtime/${showtimeId}`).pipe(
      map((res) => res.data),
      catchError(this.handleError<Showtime>('showtime'))
    )
  }
  ...
  getAvailableSeatByShowtime(showtimeId: number) {
    return this.http.get<ApiRes<Showtime>>(`${this.URL_SHOWTIME}/showtime/${showtimeId}/available`).pipe(
      map((res) => res.data),
      catchError(this.handleError<Showtime>('showtime'))
    )
  }
