...
import {selectMovieLoading, selectMoviesError,} from '@/app/core/store/state/movie/movie.selectors';
...
export class ShowtimeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.store.select(selectAllTheaters)
      .pipe(
        filter(t => t.length > 0),
        take(1),
    ...
    this.form.valueChanges.pipe(takeUntil(this.destroy$)).subscribe(() => this.loadData());
  }
  ...
  loadData() {
    ...
    this.showtimes = [];
    this.store.dispatch(ShowtimeActions.loadShowtimesByTheaterId({theaterId: theaterId}));

    this.store.select(selectAllShowtimes)
      .pipe(filter(showtimes => !!showtimes && showtimes.length >= 0), debounceTime(100), takeUntil(this.destroy$))
      .subscribe((showtimes) => {
